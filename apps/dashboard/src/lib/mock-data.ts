// Realistic but entirely invented sample data — no PHI, no real entities.
// Mirrors window.SCHISSEL_DATA from docs/design-bundle/source/data.js.

export type StateStatus = 'active' | 'progress' | 'expiring' | 'none';
export type PayerStatus = 'notstarted' | 'submitted' | 'review' | 'approved';
export type EngStatus   = 'active' | 'hold' | 'prospect' | 'ended';
export type CheckStatus = 'notstarted' | 'progress' | 'done';
export type TrendDir    = 'up' | 'down' | 'flat';

export interface MockState {
  code: string; name: string; status: StateStatus;
  date: string | null; imlc: boolean; home?: boolean;
}
export interface MockPayer {
  name: string; type: string; status: PayerStatus; date: string | null;
}
export interface MockEngagement {
  name: string; model: string; volume: string; rate: string; status: EngStatus;
}
export interface MockCheckItem {
  task: string; group: string; status: CheckStatus; date: string | null;
}
export interface MockMonth { m: string; rev: number; exp: number; partial?: boolean; }
export interface MockExpenseCategory { tag: string; label: string; amount: number; }
export interface MockKpi {
  id: string; label: string; value: string | number;
  sub: string; trend: TrendDir; trendLabel: string; section: string;
}
...
export const MOCK_PAYERS: MockPayer[] = [
